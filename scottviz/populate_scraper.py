...
def populate_msps():
    with open('../scraper/msp_scraper/msps.csv', mode='r') as infile:
        ...
        for row in reader:
            ...
            m = MSP(firstname=row[1].strip(), lastname=row[0], constituency=c, party=p, foreignid=i, member_startdate = parser.parse('5 May 2011').date(), member_enddate = parser.parse('5 May 2016').date())
            name = str(m)
            if name in msp_img_urls.keys():
                m.img = msp_img_urls[name]
            if name in msp_job.keys():
                job_info = msp_job[name]
                m.job = job_info[0]
                m.job_startdate = job_info[1]
                m.job_enddate = job_info[2]
            m.save()
...
def populate_votes(files):
    ...
    # That means taking into account the dates between which he was a member
    for msp in msps:
